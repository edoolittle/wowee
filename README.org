...
1a. Go through keybinds-global.ahk and remove hyper key bindings for applications
    which are not installed.  In other cases, type Win+R, shell:appsfolder, and drag
    an app .lnk file to a directory on your Windows path.  Then double click the .lnk
    file to make sure it works; you may have to uncheck a box asking for permission to
    run every time.
